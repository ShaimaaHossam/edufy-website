...
function MainForm({ formType }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(setStepData({ step: 1, data: { data: values } }));
  ...
  useEffect(() => {
    !propertyTypeID && setFieldValue("property_subtype_id", "", false);
  ...
  // adding step errors to formik
  const { canLeave, stepsData } = useSelector(stepperSelector);
  useEffect(() => {
    !!stepsData[1].errors && setErrors(stepsData[1].errors);
  }, [stepsData, setValues, setErrors]);

  const [leaveDialogOpen, setLeaveDialogOpen] = useState(false);
  // if form is dirty and didn't approve leaving, BLOCK
  ...
  return (
    ...
    >
      ...
      <Grid item container spacing={3}>
        ...
        <Grid item xs={12}>
          <NumberInput
            required={formik.values.wallet_type_id === WALLET_TYPES.restricted}
            disabled={formik.values.wallet_type_id === WALLET_TYPES.unlimited}
            name="wallet_amount"
      ...
      <Grid item container spacing={3}>
        ...
        <Grid item xs={12}>
          <Autocomplete
            ...
            onBlur={formik.handleBlur}
            error={
              formik.touched.area_manager_id && !!formik.errors.area_manager_id
            }
            helperText={
              formik.touched.area_manager_id && formik.errors.area_manager_id
            }
      ...
      <Dialog
        ...
        onConfirm={() => dispatch(setCanLeave(true))}
      >
